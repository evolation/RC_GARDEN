...
HAL_StatusTypeDef FLASH_EXT_If_Write(void  *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  if (BSP_FLASH_Write(0, (uint8_t *) pSource, (uint32_t) pDestination - EXTERNAL_FLASH_ADDRESS, uLength) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
...
/*   while (BSP_FLASH_GetStatus(0) !=  BSP_ERROR_NONE) */
  ...
  if (BSP_FLASH_Read(0, (uint8_t *) pDestination, (uint32_t) pSource - EXTERNAL_FLASH_ADDRESS, uLength) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
