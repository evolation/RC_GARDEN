...
/*
 * heap base and heap limit are required during compilation due to the
 * implementation of free() provided in malloc.c.
 * These symbols are not used and stripped by the linker since
 * the application does not make use of the function calling the free()
 */
uint32_t __heap_base = 0x0;
uint32_t __heap_limit = 0x0;
